...
namespace CrystalMir2
{
    ...
    public class MapReader
    {
        ...
        private void CombineJson()
        {
            string dir = Path.GetDirectoryName(FileName);
            string jsonName = Path.GetFileNameWithoutExtension(FileName);
            string path = Path.Combine(dir, jsonName + ".json");
            if (File.Exists(path))
            {
                string json = File.ReadAllText(path);
                var mJsonData = JsonTool.FromJson<MapJsonClass>(json);

                for (int i = 0; i < mJsonData.Width; i++)
                {
                    for (int j = 0; j < mJsonData.Height; j++)
                    {
                        MapCells[i, j].Attribute = (int)mJsonData.Cells[i, j].Attribute;
                        MapCells[i, j].FishingAttribute = mJsonData.Cells[i, j].FishingAttribute;
                        MapCells[i, j].BackImage = (MapCells[i, j].BackImage & 0x1FFFFFFF) - 1;
                        MapCells[i, j].MiddleImage = MapCells[i, j].MiddleImage - 1;
                        MapCells[i, j].FrontImage = (MapCells[i, j].FrontImage & 0x7FFF) - 1;
                    }
                }
            }
        }
